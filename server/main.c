...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        // Once a connection is accepted, fork the main process
        pid_t pid = fork();
        ...
        // In child process, close unused sfd descriptor, and process the client connection
        close(sfd);
...
void handle_client_connection(int cfd)
{
    ...
    int buffer_at = 0, read_n = 0, header_delimiter_found = -1, header_delimiter_size = 0;
    // First of all, to handle a client connection, we need to parse the headers
    // For simplicity, let's limit their size up to BUFFER_SIZE (currently 4 KiB)
    // Such limit is widely used among known HTTP servers
    while (buffer_at < BUFFER_SIZE)
    {
        ...
        // After reading new data, try to find a header separator
        for (int i = buffer_at; i < buffer_at + read_n - 3; i++)
        {
            ...
            // We should also check for LF LF, to be tolerant, as according to RFC
    ...
    // Ensure that we found the headers in the first BUFFER_SIZE bytes - if not, exit
    if (header_delimiter_found == -1)
    {
    ...
    while (1)
    {
        ...
        // Check if header line was found, and ensure it isn't empty
        if (header == NULL)
        {
            break;
        }
